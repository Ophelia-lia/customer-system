...
engine = create_engine(db_url, connect_args=connect_args, echo=True)  # 添加 echo=True 用于调试
...
def save_all_data(data: Dict[str, Any], session: Session = Depends(get_session), current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        # 正确的删除所有客户数据的方式
        print(f"准备保存 {len(customer_list)} 条客户数据")
        existing_customers = session.exec(select(Customer)).all()
        print(f"删除现有的 {len(existing_customers)} 条数据")
